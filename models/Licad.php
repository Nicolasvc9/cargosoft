...
class Licad extends Base
{
    ...
    public function getConsecutivoLicad($ruta)
    {
        $response = $this->mysqli->query("SELECT * FROM consecutivos_licad WHERE idciudad = (SELECT idcity_origin FROM routes WHERE id = $ruta)");
        return $response->num_rows > 0 ? $response->fetch_assoc() : false;
    }
    public function getPallets()
    {
        $response = $this->mysqli->query("SELECT pallet, SUM(kg) AS total_kg FROM guides WHERE licad = '0' GROUP BY pallet;");
